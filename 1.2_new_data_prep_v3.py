...
for c in  file_df['category'].unique():
    ...
    categry_df = get_df_in_v2(categry_df, c,columns_dict, features_group)
    categry_df = add_period_lbls(categry_df,periods,time_period_type)
    chk_na = categry_df.isna().sum()
    if chk_na[chk_na > 0].shape[0] > 0:
        print('WARNING\n', 'columns with na:')
        print(chk_na[chk_na > 0])          
    i= categry_df.shape[0]
    categry_df[[i for i in columns_dict.values() if i in categry_df.columns]] = \
    categry_df[[i for i in columns_dict.values() if i in categry_df.columns]].round(3)
    categry_df = categry_df.drop_duplicates()
